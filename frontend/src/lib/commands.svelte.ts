...
export const COMMANDS: Command[] = [
  ...
  { id: 'preview_gp_growth', label: '看GP候选长出什么', group: '专家', run: () => { setMode('expert'); setWorkspace('gp'); store.gpGrowSignal++; return '已在进化(GP)工作区展开当前选中候选的「看它长出什么」3D 生长预览（绿边=GP 新长出的依赖、节点脉冲=方程形式变了）。需先跑出 GP 候选并选中一个。' }, keywords: 'gp growth 生长 长出 新枝 diff 进化 candidate grow', access: 'read', description: '展开 GP「看它长出什么」彩蛋(GA-6b Phase 3)：在进化工作区当前选中的 Pareto 候选上，于 3D 结构里播放采纳它会带来的结构变化——绿色「新枝」= GP 新长出的依赖边、节点脉冲 = 方程形式改变。需先在进化工作区跑出候选并点选一个。' },
