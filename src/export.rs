...
/// 子系统名 → 旁白（草莓/番茄/蓝莓/温室各模型的子系统；键按名匹配，共享名写得通用、
/// 各家独有写专属；未命中回退通用句）。改这一处即改全部字幕。
fn growth_narration(key: &str) -> String {
    let s = match key {
        // —— 作物共享（草莓/番茄/蓝莓） ——
        "光合" => "长出光合引擎：把光、CO₂ 和温度转成糖，这是干物质的来源。",
        "温度" => "温度：冷热怎样驱动各个过程的快慢与平衡。",
        "物候" => "接上物候时钟：按温度积累（积温/需冷）推进发育，决定何时萌动开花。",
        ...
        "钙" => "接上钙：钙随蒸腾流进果实，不足会引发脐腐（品质风险）。",
        "碳分配" => "搭起碳分配：把光合来的糖按需求分给各个器官（呼吸要扣掉）。",
        "品质" => "接上品质：算出糖度（Brix）——高 EC/亏水让糖往果里浓缩，果更甜但更小（产量-品质权衡）。",
        // —— 番茄特有 ——
        "叶茎" => "铺开叶与茎：叶截光、茎根支撑，结构骨架长起来。",
        // —— 蓝莓特有（多年生灌木） ——
        "冠层光合" => "长出冠层光合：叶面积截光、把光合成糖（扣掉维持呼吸）。",
        "多年生储备" => "接上多年生储备：蓝莓是多年生，糖多了存进根冠、不够再调出来。",
        "器官生长" => "各器官生长：叶/枝/根/冠按分到的碳长大，老叶老根会衰亡。",
        "果实产量" => "挂上果实产量：开花×需冷定果数，双 S 曲线长大，最后得产量。",
        // —— 温室（环境物理） ——
        "辐射" => "先接太阳：算进多少阳光透过覆盖材料进到温室里。",
        "通风" => "打开通风：天窗/风机把多余的热量和湿气换出去。",
        "CO2" => "接上 CO₂：作物吸收 + 通风换气共同决定棚内 CO₂ 浓度。",
        "湿度" => "管住湿度：蒸腾加湿、通风除湿，决定棚内空气有多潮。",
