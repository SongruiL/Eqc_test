...
/// 前端 v2（**正统**，`docs/spec-studio-frontend-v2.md`）：`frontend/` 的 Vite+Svelte+TS 源码
...
fn handle(mut stream: TcpStream, ctx: &Ctx) -> std::io::Result<()> {
    ...
    let (status, ctype, body): (&str, &str, String) = match route {
        // v2（正统）：Vite+Svelte 构建的单 HTML，消费 live /api/*。v1 studio.html 已退役——/ 与 /v2 统一发 v2。
        "/" | "/index.html" | "/v2" => ("200 OK", "text/html; charset=utf-8", STUDIO_V2_HTML.to_string()),
        "/__version" => (
            "200 OK",
