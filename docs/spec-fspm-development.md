...
**验证**：validate 721 方程；**守恒 C_system 层与 F4 baseline 的 C_total 层逐位相同**（含早期瞬态、含后期精确 → 证采收账精确抵消·零回归）；采收动态=在株果 step40 见顶 14.16→采收启动→step80 采空、累计采收阶梯升到 21.74；总产出 `Y_total=21.74=F4 的 Y`（采收不改总产出）。

**★诚实标注**：①F4 实测坐果数订正（旧"43/9"是把产量 Y 误记为坐果数）——理想光 set_count=33.2·Y=21.7、弱光 set_count=21.2·Y=9.0。②**早期 Cbuf 耗尽瞬态**（残差 71/相对 5e-4，**F4 遗留·非 F5a 引入**，与 baseline 逐位相同）：开季果抽碳>弱光合→缓冲库撞底（显式 Euler 在库存硬约束边界的经典瞬态）；留 F5c 守恒诊断 + 标定修。③6 节 determinate 下采完即空、Cbuf 后期虚胀（step149=79356）——F5b indeterminate 才消。

### F5b 扩规模 + 引擎 pass 迭代化（待施工）

metamer 6→25（×8 花=200 果）、模拟期拉满整季；引擎递归 pass（拓扑排序/eval）改迭代、不靠堆栈（F4 当时靠 64MB 栈绕过，~200 果要正面解决）；配 F5a 采收→稳态挂果数月、buffer 不虚胀、Q/D 稳态。验证：cargo test 绿×2、200 果不溢栈、守恒守住、稳态现象（持续坐果+采收）。

### F5c 标定接口夯实（待施工）

守恒诊断 CLI（`eqc simulate --check-balance`：自动核 d(C_system)=A_gross−resp 容差、捕早期瞬态）+ measurable 量审计（田间能测的量都标 + 单位口径对齐云南：株高/LAI/坐果数/单果重/累计采收）+ provenance 选靶确认（可标定参数 推导/猜测 全标·喂受约束 GP）。
