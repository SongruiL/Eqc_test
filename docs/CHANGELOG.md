...
- **验证**：svelte-check 0 错；两 feature 配置 **288 lib 绿**（+2 palette 测试：向白混合 / 声明顺序分槽去重）；Playwright 3 passed（+1 条 2D 配色切换改 iframe `color=` 参数）；2D 类别/子系统 + 3D 三态截图肉眼确认（2D 浅、3D 鲜、同色相、Forrester 形状+中文名）。**遗留**：8 类 Forrester 色仍在 Rust CSS + TS 两处（低频，未纳入本轮单一真相源；模块色已统一）。
...
- **唯一剩余 = GA-6b（GP 生长动画）**：GP 采纳/进化时按 GA-4 diff 播"长出新枝"——需 GP arc 活跃 + 两版本 diff 端点；待 GP 活跃或用户想要时做。
