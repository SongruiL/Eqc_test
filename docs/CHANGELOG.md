...
- **施工进度**：**第1步 ✅**（commit `58b28bb`）AST `Expr::Aggregate{kind:ReduceKind, over:TopoSelector, of, body}` + `{agg:…}` 反序列化 + 6 处穷尽 match（to_python/to_rust 守 `unreachable!`、to_latex 显 Σ）+ 单测，lib 293 绿。**第2步 ✅** 加载期 lower（`structure_expand`：`over: children` 取父的子实例集、`over: all` 取实体全集 → 每目标实例化 body 折 add/mul 链；`RefCtx.inst` 改 `Option` 让整株共享方程也能 `over: all`；`mean` 空集 count=0 加载期报错、`sum` 空集=0），lib 295 绿（+2 lower 单测）+ 端到端 fixture：`eqc validate` 10 方程（聚合已 lower）、`simulate` 数值正确（节级 Σ children=0.2、全株 Σ all=0.6，积分对）。**第3步 ✅** cohort 收编采 **A·单一折叠源**（施工时发现 cohort `sum_over` 早已 YAML 层折链、且 `fold_binary` 与 structure 重复、pass 顺序 structure 在 cohort 前）：抽 `src/parser/agg_fold.rs`（`fold_binary`/`op_args`/`fold_sum_or_prod`），cohort `sum_over/prod_over` + structure `sum/prod` 共用、删两份重复；297 lib 绿（+2 agg_fold 单测）+ 草莓 S8 仿真 `Y=7.558441109220954`（既知值、逐位不变）。**第4a步 ✅·聚合可见性（后端+契约）**：`StructureInfo` 加 additive `aggregations: Vec<AggregationInfo{output,kind,over,entity?}>`（空→skip，旧结构模型契约逐字节不变）；`structure_expand` 展开时扫 `{agg}` 采集出处→写 structure 块；`export.rs` 合并带出→`ModelJson.structure.aggregations`；`contract.ts` 镜像。297 lib 绿（含出处断言）+ `eqc export` 契约含 `aggregations`（nf_rate sum/children、pf_rate sum/all/fruit）。仅采 structure 路径（cohort sum_over 出处未采，作 follow）。**第4b步 ✅·前端**：`Topology3d.svelte` 3D 拓扑图例据契约 `structure.aggregations` 加「Σ 聚合关系」段（node_fruit Σ·各子器官 / canopy_leaf Σ·全 metamer / plant_fruit Σ·全 fruit）；svelte-check 0 + npm build + cargo embed 重建 + 真 Studio 截图确认 + Playwright `fspm_structure` 加 Σ 断言；tomato_fspm demo 加聚合方程（本地）。**★风险3（拓扑聚合算子）全部完成：算子(1)+加载期 lower(2)+cohort 收编(3)+契约/前端可见性(4a/4b)。**

### FSPM 功能层② 器官流 / 器官级源-库碳经济 —— v1 完成 ✅（风险4）
把番茄整株源-库（Vanthoor/De Koning 箱车）下沉到**单果分辨率**。spec `docs/spec-fspm-organ-flow.md`；**先做文献 deep research** 定向（器官级共识 = 共同同化物池 Heuvelink 1995 + 相对汇强=潜在生长率全局分配；现代 FSPM 弃箱车、改每果热龄 + beta Yin 2003；见 `crop-models/tomato/番茄FSPM模型文献综述.md`）。
- **★架构论断坐实**：器官多源流**没加任何引擎特性**——风险1 实例 + 风险3 聚合 + rate 组合表达式即可表达，**守恒由聚合天然保证**（缓冲库排出 = Σ各器官分配、引用同一聚合量、单一真相源漏不掉）。仅加 `{rank}` 一个小特性 + 修法D 一个上规模硬化。
- **step1 `{rank: self|parent}`**（`04a57e0`）：`structure_expand` 加载期把实例序号折常量（实例 id 末段路径分量，类比 cohort `{idx}`），用于器官错峰出现阈值 θ。
- **step2 门控激活 + 每果热龄**（`30bc0c3`）：纯模型 + 测试、**零引擎改动**；每果 θ=(节位−1)·phyllo+(果位−1)·ψ 错峰出现、出现后才累积热龄（坐实"多源流不需新引擎特性"）。
- **step3 器官碳经济 + 交付模型**（`1673633`/`a462580`）：共同池 `Ap=Anet+Cbuf_prev` + 相对汇强 `ssp=Σss` + 每果 Yin beta `cw(age_i)` 汇强 + 分配 `A_i=ss_i·share` + 缓冲库 `rate_Cbuf=Anet−ΣA`；lib 测试核守恒 `|d(ΣC)−(A_gross−呼吸)|<1e-9`。交付模型 `crop-models/tomato/tomato_fspm.eq.yaml`（6 节×4 果=24 单果）。
- **★修法D 聚合扁平化**（`343d80c`）：24 果暴露递归 pass 深度 **O(器官数)→栈溢出**（根因=聚合 Σ over N 在 `agg_fold` 折成 N 深左嵌套 add 链）。修=lower 成扁平 `vsum`/`vprod` over `vector`（深度恒 1），复用现成 `Reduce`+`VectorLit`、零新变体。**逐位不变**（`Reduce(Sum)=iter().sum()` 从 0.0、IEEE `0+x=x` 精确 → 与旧链逐位同，cohort S8 锚 `Y=7.558441109220954` 不破）。备选 A 平衡折叠破逐位锚、B 递归改迭代工作量大，均否。**FSPM 上规模（叶器官化/真 truss/多茎）地基**。更新 cohort/structure 两处「断言 add 链」测试为 vsum/vector。lib `--features cli` 301 绿。
- **Studio 验收**：重建 eqc.exe → `eqc validate`（222 方程）/`structure`（🌿 fruit×24+metamer×6 非奇异）/`simulate`（守恒 4e-11、逐果碳错峰）真实 CLI 全过；契约 `/api/model` 带 structure.entities + 5 条 aggregations；Studio /v2 3D 拓扑图例显「🌿器官结构 + Σ聚合关系」、仿真标签画逐果碳错峰 S 曲线（截图确认）。
- **deferred**：守恒诊断 CLI（`--check-balance`）、带移除采收流、GreenLab 概率坐果（连续比例）、条件聚合（mean over active）、叶/节间器官化 + 逐叶截光（绑风险5）、完整 FvCB/水N EC Ca 外壳平移、真 truss（`borne_on`/`at`+`{rank:grandparent}`）。**下一步 = 风险5 几何/glTF（GIS B2 真植株 3D，器官碳已就绪），引擎选型另议。**

### 模型进化图论 arc — 数据层 + 分析层 + 呈现层 Tier1/Tier2 + 单一真相源闭环（全部部署上线）
把「模型进化史」从隐式（文件名 s1..s8 后缀 + 散记）升级成一等公民、可视、可回退、可图论分析的进化系统。spec = `docs/spec-model-evolution-arc.md`。
- **数据层**：`schema::Lineage{parent,step}` + `Metadata.lineage`（additive，读入之前 serde 忽略的 `meta.lineage`）；契约 `ModuleJson` 导出 `version`+`lineage`（schema_version 不动）。
- **分析层**：`src/evolution.rs` + CLI `eqc evolution`——走血缘链 → `git show` 逐版本取历史源码 → **统一 all-output 口径** → `graph::{analyze_metrics,identifiability,structure}` 逐版本 + `diff_models` 相邻版本 → `EvolutionReport`（轨迹 + 版本 diff + **标定坑清单**：混淆系数簇 + 阈值不可辨识，机理归因自动派生自共享方程中文名 + 诚实白名单可辨识性）。零核心改动。
- **★三链普适验证**：草莓 s1→s8.1 / 番茄 t1→3.1 / 蓝莓 bb1→bb5，「混淆团=经验响应式系数簇」跨三作物两建模范式全部成立；加了 cohort/箱车展开兄弟簇过滤（`is_last__N` 那种固定标记降为脚注）。
- **呈现层 Tier1**：serve 三端点 `/api/evolution`+`/api/history`(git log)+`/api/source?rev=`(git show 历史源码)；Studio「进化史」工作区（`Evolution.svelte`：指标轨迹 SVG + 版本 diff + ★坑清单 + 诚实白名单）；🎬 演化回放动画（Topology3d 加 `reveal` prop 沿血缘逐版本「长出」，隔离 store.growth；2D Forrester 报告同步，`reveal2d.ts` 共享 helper，Structure 子系统生长重构复用同一套）。
- **Tier2 版本回退/checkout**：进化史加「本文件版本历史」git 列表 + 查看历史源码 + 人在环回退（复用 `write_source` 校验+备份+原子写·零后端改动）。
- **★单一真相源闭环**：`eqc evolution` 现可吃**模型文件**——`analyze_input` 自动识别清单/模型；`analyze_model_lineage` 递归走 `meta.lineage.parent` 链自动派生血缘。优先级=清单(老作物全谱系)>meta.lineage 自动派生(向前演进·加作物写 meta.lineage 就零手工纳入)>优雅提示。`find_repo_root` 提 pub 单一真相源。
- **Tier3 spec 定稿**（`docs/spec-gp-candidate-branching.md`，待开工）：GP 候选分支化 + 图论硬过滤（加环/破守恒/引入新混淆对机械淘汰）+ Claude Code 开发会话多准则采纳到 `gp/<模型>/<靶点>` 分支（自主）、转正合 main（首席拍板）。
- 两配置 `cargo test` 311 lib 全绿全程；对抗双审（数据/分析层）通过；真浏览器 + e2e 验证；release 重建冒烟（FSPM 防崩）+ systemctl restart 部署仪式。
