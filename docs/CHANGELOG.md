...
- **施工进度**：**第1步 ✅**（commit `58b28bb`）AST `Expr::Aggregate{kind:ReduceKind, over:TopoSelector, of, body}` + `{agg:…}` 反序列化 + 6 处穷尽 match（to_python/to_rust 守 `unreachable!`、to_latex 显 Σ）+ 单测，lib 293 绿。**第2步 ✅** 加载期 lower（`structure_expand`：`over: children` 取父的子实例集、`over: all` 取实体全集 → 每目标实例化 body 折 add/mul 链；`RefCtx.inst` 改 `Option` 让整株共享方程也能 `over: all`；`mean` 空集 count=0 加载期报错、`sum` 空集=0），lib 295 绿（+2 lower 单测）+ 端到端 fixture：`eqc validate` 10 方程（聚合已 lower）、`simulate` 数值正确（节级 Σ children=0.2、全株 Σ all=0.6，积分对）。**第3步 ✅** cohort 收编采 **A·单一折叠源**（施工时发现 cohort `sum_over` 早已 YAML 层折链、且 `fold_binary` 与 structure 重复、pass 顺序 structure 在 cohort 前）：抽 `src/parser/agg_fold.rs`（`fold_binary`/`op_args`/`fold_sum_or_prod`），cohort `sum_over/prod_over` + structure `sum/prod` 共用、删两份重复；297 lib 绿（+2 agg_fold 单测）+ 草莓 S8 仿真 `Y=7.558441109220954`（既知值、逐位不变）。余 **第4步** 契约 additive（可选/并入风险4）。
