...
- **下一步**：Phase 3（可选彩蛋：`/api/diff` 包 GA-4 `diff_models` + 真 GP 一步「长出新枝」，GP 采纳时按 before/after diff 高亮渐入；GP 已可跑——`/api/evolve`+`gp::patch_model`+gpdemo 合成数据，无需田间数据）；v2（跨版本锚定布局做平滑 morph）。
