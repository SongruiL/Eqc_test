...
**模型图论分析 arc — 后端 GA-1~5 全部完成（schema_version 仍=1，老前端零回归）**
- 已落地：`src/graph/` = `bipartite + matching + dm`（GA-1 结构地基）、`digraph + identifiability`（GA-2 可辨识性）、`metrics`（GA-3 网络指标）、`diff`（GA-4 版本 diff）、`layout3d`（GA-5 3D 坐标）。CLI 统一在 `eqc structure [--identifiability|--metrics|--layout3d]` + `eqc diff`；契约全是 `StructureJson`/`GraphDiffJson` 的 additive 字段。全部纯 Rust、数据无关、确定性、合成+真模型对拍。
- **唯一剩余 = GA-6（前端 3D 渲染 + GP 生长动画）**：单独 Studio 子 arc，three.js/threlte 消费 GA-5 坐标 + GA-3 指标渲染；GP 采纳时按 GA-4 diff 播"长出新枝"。首次碰前端——须守★标准约定（前端可操作功能在 `frontend/src/lib/commands.svelte.ts` 登记命令）。
