...
>   - **生长演示（GA-6b）**：头部「▶ 生长演示」按作者子系统（meta.modules）**声明顺序**把模型一块块「长出来」（base 输入 → 光合 → 温度 → …），配**旁白字幕**（非专家能懂）。给园区/甲方看模型怎么搭起来的演示——**不需要田间数据**（演示的是结构生长、不是数据拟合）。可播放/暂停/单步/退出；3D 节点 `scale 0→r` 平滑长出、边按揭示逐章接上。⌘K 命令 `play_growth` 可直达。（Phase 2 将让 2D 报告同步。）
