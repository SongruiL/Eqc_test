...
>   - **配色可切换 + 常驻图例**：3D 头部「按类别 / 按子系统」切换。**按类别**（默认）= Forrester 动力学类别（存量/速率/驱动…）；**按子系统** = 作者在 `meta.modules` 声明的子系统（草莓的光合/氮/水…、温室的辐射/通风/温度…），一眼看出模型分几大块（模型没声明子系统则该项禁用）。左下角**可折叠图例**只列当前模型出现的项，按类别每项带一句话含义（非专家能懂）。配色为半立体（自发光底色+暗光，颜色不被光照冲淡）、选中=白描边光环。命令 `set_topology_color_by`（`mode: class|module`）可直达。
