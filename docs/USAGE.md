...
- **目标/约束 = S 表达式**，复用解释器，领域无关。其「变量」是对整段轨迹的**时间归约**：`final/at/max/min/mean/total`（区别于逐日算子与 `vsum`）；还能引用旋钮值与常量（成本项要用）。约束 `expr ≤ max` 用惩罚法。
- **优化器**：差分进化 **DE**（免导数、对非光滑/阈值/多峰鲁棒、定种子可复现；垃圾候选给最差值不崩）。用 `eqc sweep --sensitivity` 作优化前预筛（哪些旋钮值得纳入）。
- 草莓 S4 实测：最大化产量 → CO₂/Pd 顶界 Y=10.95 kg/m²；利润变体（CO₂ 有成本）→ 最优 CO₂≈757 ppm（内点）。Pd 最优与 `eqc sweep` 网格逐位一致；最优点用独立 `eqc simulate` 复现逐位一致。
...
下一步备选：优化层阶段 2（约束可视化、`--sensitivity` 自动预筛接入、收敛/最优可视化）、阶段 3+（时变控制曲线参数化、离散旋钮、参数标定接田间数据、CMA-ES/贝叶斯）；**GP 约束进化层**（核心愿景，复用目标评估核当 fitness 引擎，fitness=跑仿真 vs 实测，需先讨论可进化节点/约束）；矩阵 eval（V4）、cohort 在图上分组显示、报告增强、耦合的时间尺度聚合。
