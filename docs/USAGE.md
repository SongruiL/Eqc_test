...
> - **3D 拓扑视图（GA-6）**：结构工作区右上「2D 报告 ↔ 3D 拓扑」切换（**2D 默认**）。3D 用 three.js 渲染依赖图的 3D 力导向布局——节点球大小∝介数中心性（枢纽更大）、颜色=Forrester 类别（复用 2D 配色）；可轨道旋转/缩放/平移、悬停看注释、点选与 2D/仿真视图联动。坐标由 EQC-Rust 算（`/api/layout3d`）、前端只渲染。3D 需 WebGL（Studio-only，离线报告仍零 JS）。⌘K 或「问AI」用命令 `view_topology_3d` 可直达。
