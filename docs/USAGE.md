...
>   - **2D Forrester 合并（形状=类别 · 颜色=子系统）**：变量级 Forrester 按子系统配色时——**形状仍编码类别**（存量矩形/速率阀门/驱动椭圆/半状态虚框…，保留虚实线），**填充色 = 子系统**，主标签用**中文名**。旧的独立「依赖关系图(DAG)·按子模块分色」已并入此模式（不再单独画）；「方程」粒度退役（保留「变量」「模块」两档）。
