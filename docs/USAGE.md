...
> - 端点：`/api/model`（JSON 契约）、`/api/report[?layout=force]`（HTML 报告）、`/api/layout3d?model=`（3D 拓扑力导向坐标 JSON，GA-6 前端 3D 视图消费）、`/api/simulate`（轨迹 JSON）、`/api/chart.svg?vars=Y,TDM`（折线图 SVG）、`/api/optimize?spec=problem.yaml`（跑优化、返回最优旋钮+收敛轨迹+收敛曲线 SVG）、`/api/models`（多模型工作区花名册，前端建选择器用）、`/api/llm`（前端 AI 助手代理 Claude，见下）。
...
> - **3D 拓扑视图（GA-6）**：结构工作区右上「2D 报告 ↔ 3D 拓扑」切换（**2D 默认**）。3D 用 three.js 渲染依赖图的 3D 力导向布局——节点球大小∝介数中心性（枢纽更大）；可轨道旋转/缩放/平移、悬停看注释、点选与 2D/仿真视图联动。坐标由 EQC-Rust 算（`/api/layout3d`）、前端只渲染。3D 需 WebGL（Studio-only，离线报告仍零 JS）。⌘K 或「问AI」用命令 `view_topology_3d` 可直达。
>   - **配色可切换 + 常驻图例（2D/3D 统一）**：「按类别 / 按子系统」切换在 **2D 变量级 Forrester 与 3D 拓扑共用同一开关**。**按类别**（默认）= Forrester 动力学类别（存量/速率/驱动…）；**按子系统** = 作者在 `meta.modules` 声明的子系统（草莓的光合/氮/水…、温室的辐射/通风/温度…），一眼看出模型分几大块（模型没声明子系统则该项禁用）。配色单一真相源在 EQC-Rust（`src/palette.rs`，声明顺序定色），**同一子系统在 2D（浅调）与 3D（鲜调）同色相**。3D 左下角**可折叠图例**只列出现项、按类别带一句话含义；3D 为半立体（自发光底色+暗光）、选中=白描边光环。命令 `set_topology_color_by`（`mode: class|module`）可直达。
>   - **生长演示（GA-6b）**：头部「▶ 生长演示」按作者子系统（meta.modules）**声明顺序**把模型一块块「长出来」（base 输入 → 光合 → 温度 → …），配**旁白字幕**（非专家能懂）。给园区/甲方看模型怎么搭起来的演示——**不需要田间数据**（演示的是结构生长、不是数据拟合）。可播放/暂停/单步/退出；3D 节点 `scale 0→r` 平滑长出、边按揭示逐章接上。**2D 报告与 3D 拓扑同步**（同一份 plan + 控件 + 旁白；切视图保持进度），在当前视图开演。⌘K 命令 `play_growth` 可直达。
>   - **2D Forrester 合并（形状=类别 · 颜色=子系统）**：变量级 Forrester 按子系统配色时——**形状仍编码类别**（存量矩形/速率阀门/驱动椭圆/半状态虚框…，保留虚实线），**填充色 = 子系统**，主标签用**中文名**。旧的独立「依赖关系图(DAG)·按子模块分色」已并入此模式（不再单独画）。
>   - **三档粒度**：「变量」=完整 Forrester（含参数叶子）；「方程」=**计算骨架**（隐去常数参数叶子，留驱动+变量+方程；同款 Forrester 形状+可配色，看"算什么→喂给谁"）；「模块」=折叠子模块超级节点。三档都支持 Forrester/力导向/分层三种布局。
...
>   - **「🌱 看它长出什么」（GA-6b Phase 3 彩蛋）**：候选块上的按钮——在 3D 结构里播放采纳此候选会带来的**结构变化**：GP 新依赖的输入边以**绿色「新枝」从源点伸出长成**、形式变了的方程节点**脉冲**，配旁白（如「把门控从 linear_ramp 改成互作形式，长出新枝 d2→门控」）+ 结构编辑距离/新增边数。受约束 GP 不引入新变量，故"新枝"=边重连+形式变；**单输入靶点**（如 `gpdemo`）diff 极小=纯形式脉冲（配 rediscovery 徽章），**多输入靶点**才长真边（合成 demo `gpdemo3` 双输入互作门控、或蓝莓 BB5 的 `ChillAccum/GDD` 休眠门控）。这正是"GP 提议 → 看结构 diff + 溯源 → 采纳进单一真相源"的审查 UX。⌘K/「问AI」命令 `preview_gp_growth` 可直达（先跑出候选并选中一个）。每候选的 `structure_diff`（GA-4 `GraphDiffJson`）随 `/api/evolve` 结果带回、前端零延迟播。**2D 报告暂不画新边**（按 before 渲染；留 follow-up）。
