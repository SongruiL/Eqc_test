...
> - 验证：两配置 **288 lib 绿**（+2 palette 测试）、Playwright +1（2D 配色切换改 iframe `color=`）、截图肉眼（2D 浅/3D 鲜同色相、Forrester 形状+中文名）。
>
> **收尾：方程级骨架复活 + 8 类配色统一 ✅**：
> - **方程级复活**：变量级与方程级同款走 `forrester_svg`（Forrester 形状+可配色），方程级吃 `collapse_to_equations`（藏参数叶子=计算骨架）；Forrester/力导向/分层三布局全支持。前端 `levels` 加回「方程」。（误退役教训：把"冗余的依赖图"和"方程粒度"绑一起切，后者"藏参数=骨架"是独立价值。）
> - **8 类色搬进 palette.rs**：`class_color→{vivid,fill,stroke}`（显式三元组，边界白云/控制品红等手调约定纯派生会坏）；2D 报告 `.fsh`/图例由 `forr_class_css()` 从 palette 生成（width/dash 留报告）；契约 `ModelJson.class_colors` 暴露鲜调，前端 `classColor3d` 优先读契约。**2D/3D 配色（类别+子系统）至此全单一真相源、零两套维护。**顺带修上轮 deferred 的 2D 辅助/参数（冷/暖拉开）。
- 立场重申：**2D（现有 SVG 报告/Forrester）仍是默认分析视图**；3D 是总览 + 生长叙事的补充。3D 需 WebGL（Studio-only、不进零-JS 离线报告）。
- **GA-6b 生长动画 · Phase 1（3D）✅ 已落地**：把模型按子系统 `meta.modules` **声明顺序**一块块「长出来」+ 旁白（给非专家/甲方看，**不需田间数据**——演示结构生长非数据拟合）。**主 demo = 同一张图渐进显形**（非版本 diff → 节点位置不变、无跳变）：新端点 `/api/growth?model=` 返回有序章节（base→各子系统，每章 `{title,narration,nodes(本地名)}`，本地名 = 2D `data-var`/3D `localName` 同键 → 一份 plan 同驱两视图）；旁白在 `export::growth_narration()`。3D（Topology3d）节点 `scale 0→r` 补间长出、边按揭示逐章重建；`store.growth` + 控件（播放/暂停/单步/退出 + k/N）+ 底部旁白字幕 + ★命令 `play_growth`。验证：两配置 288 lib 绿、Playwright 5 passed（+1 生长）、`/api/growth` 草莓 10 章、截图肉眼（光合/物候逐步长出+旁白）。
  - **Phase 2（2D 同步）✅ 已落地**：`Structure::applyGrowth2d()` 伸进 iframe 按**同一份 plan** 逐章给 `[data-var]`/`.fedge` 加 `opacity` 过渡显形（边=两端 `data-id` 都显形才出现），与 3D **共用 `store.growth`** + 控件 + 旁白 → 2D/3D 同步、切视图保持进度。**v1（双视图生长）完成。**
  - **Phase 3（GP「长出新枝」彩蛋）✅ 已落地**：GP 候选采纳时按 before/after 结构 diff 在 3D 里播"长出新枝"。
    - **关键认知**：受约束 GP「候选只引用 `gp_target.inputs`（已有变量）+ `__c*` 常数」→ **永不引入新节点**；"新枝" = **added 边**（用了某个原本没用的输入）+ **changed 方程**（目标式形式变）。常数代回字面值（不长 `__c` 节点）。
    - **后端（无新端点，比草案的 `/api/diff` 更省）**：evolve 结果**每候选**直接带 `structure_diff`（复用 GA-4 `diff_models(before, patched_folded)` → `GraphDiffJson`，本地名对齐）。单靶 `run_evolve_job` + 联合 `run_evolve_joint_job`（每槽 patch 自己 target）都带，sync/async 自动覆盖。前端零延迟、零回传。
    - **前端（3D 内联，复用 Topology3d）**：候选块「🌱 看它长出什么」→ 内联 `GrowthPreview.svelte`（包 Topology3d，渲染 before 模型 layout——所有节点都在，含被新边接上的输入）。Topology3d 加 `gpDiff` 模式：**added 边绿色从源点伸到目标"长出"**（独立 LineSegments，逐帧 lerp 端点）+ **changed 方程输出节点 emissive 衰减脉冲** + 旁白（前端按 `baseline.form`/`mechanistic_form`/added 边组合）。★命令 `preview_gp_growth`。
    - **诚实边界**：单输入靶点（gpdemo）diff = 纯方程形式变、零边（动画=脉冲 + rediscovery 徽章，站得住）；**多输入靶点才长真边**——新增合成 demo `gpdemo3`（双输入互作门控：真值阈值随热量 d2 漂移、现有形式只看 d1 → GP 进化到互作形式、长出 `d2→门控` 绿边；蓝莓 BB5 `ChillAccum/GDD` 靶点同理）。**2D 未做**（报告 SVG 按 before 渲染、无新边；要 2D 需重渲染 after，留 follow-up）。
    - **v2（未做）**：跨版本锚定布局做平滑 morph + 大模型按子系统聚团。
